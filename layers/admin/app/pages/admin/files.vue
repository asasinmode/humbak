...
import type { AdminVButton, AdminVDialog } from '#components';

definePageMeta({ layout: 'admin' });
useHead({ title: 'pliki - Admin' });
...
async function createDir() {
	...
	try {
		const directory = await useApi('/api/admin/directories', { method: 'post', body: {
...
async function getDirectories() {
	...
	try {
		const directories = await useApi('/api/admin/directories');
...
async function getDir(id: number | null) {
	...
	try {
		const { files, directories } = await useApi(`/api/admin/directories/${id}`);
...
const {
	...
	handleError,
	clearErrors,
} = useErrors({ deletedFileIds: {},	editedFiles: {}, deletedDirIds: {}, editedDirs: {} } as IPutDirectoryInput);
...
function getChanged(): IPutDirectoryInput {
	const deletedFileIds: IPutDirectoryInput['deletedFileIds'] = [];
	...
	const deletedDirIds: IPutDirectoryInput['deletedDirIds'] = [];
...
async function saveChanges() {
	...
	if (deletedFileIds.length || editedFiles.length || deletedDirIds.length || editedDirs.length) {
		...
		try {
			const response = await useApi('/api/admin/directories', { method: 'put', body: {
				deletedFileIds,
				editedFiles,
				deletedDirIds,
				editedDirs,
			...
			if (responseReturnsData) {
				clearLoadedFiles();
				handlePutResponse(response as unknown as IGetDirectoryResponse);
	...
	try {
		const response = await useApi(`/api/admin/directories/${currentDirId.value}`, {
			method: 'post',
			body: formdata,
		});
...
<template>
	<main id="content" class="flex flex-col gap-x-3 gap-y-5 pb-4 pt-[1.125rem]">
		<div class="mx-auto max-w-360 w-full flex gap-x-3 px-container md:px-0">
			<p class="mr-auto self-center text-lg text-neutral-5 md:ml-container dark:text-neutral-3">
				<template v-for="{ id, isActive, text } in pathBreadcrumbs" :key="id">
					<a
						:href="id ? `?dir=${id}` : ''"
						:class="isActive ? 'text-black dark:text-white underline' : ''"
						class="dark:hoverable:text-white hoverable:text-black hoverable:underline"
						@click.left.prevent="goToDir(id, $event)"
					>
						{{ text }}
					</a>
					<br v-if="!isActive" class="sm:hidden">
					<span v-if="!isActive" class="i-fa6-solid-angle-right mr-1 inline-block h-3 w-3 -ml-[0.125rem] sm:ml-1" />
				</template>
			</p>
			<AdminVButton class="hidden h-9 w-9 shrink-0 md:flex neon-blue" title="widok plików: kafelki" @click="isTiles = true">
				<span class="sr-only">widok plików: kafelki</span>
				<div class="i-fluent-grid-20-regular absolute left-1/2 top-1/2 h-[1.125rem] w-[1.125rem] translate-center" />
			</AdminVButton>

			<AdminVButton class="hidden h-9 w-9 shrink-0 -ml-1 md:flex neon-blue" title="widok plików: lista" @click="isTiles = false">
				<span class="sr-only">widok plików: lista</span>
		...
		>
			...
			>
				...
				<div
					class="relative flex basis-1/2 items-center justify-center border-t border-neutral px-3 py-4 after:(absolute text-neutral-5 font-semibold dark:text-neutral-3) before:(absolute inset-0 border-neutral border-dashed content-empty)"
					:class="[
				...
				>
					...
					<AdminVButton v-show="!isDraggingOverFiles" class="neon-blue" :disabled="computedIsLoading" @click="openFileInput">
						wgraj pliki
					</AdminVButton>
				</div>
				<AdminVLoading v-if="computedIsLoading" class="absolute inset-0 bg-black/10 dark:bg-white/10" size="40" />
			</div>
			<AdminFilesDirItem
	...
	>
		...
		<label
			...
			class="relative col-span-full flex justify-between border-2 border-neutral rounded-lg px-2 py-2"
			:class="[
		...
		>
			...
			>
			<div v-if="dir.isBeingDeleted" class="i-solar-trash-bin-trash-linear pointer-events-none absolute right-[0.3125rem] top-1/2 h-5 w-5 text-red-6 -translate-y-1/2 dark:text-red-5" />
		</label>

		<template #post>
			<AdminVButton class="justify-self-start neon-green" @click="moveGrabbedElement(dialogTargetId, true)">
				zapisz
			</AdminVButton>
		</template>
	</AdminVDialog>
