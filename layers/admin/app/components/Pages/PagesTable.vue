...
<template>
	<form class="mb-5 flex gap-x-3 md:mx-auto md:max-w-128" @submit.prevent="callGetItems(true)">
		...
		/>
		<AdminVButton class="mr-12 neon-blue" :is-loading="isLoading">
			szukaj
		</AdminVButton>
	</form>
	...
	>
		<header class="flex justify-end gap-2 bg-black/10 px-2 py-2 dark:bg-white/20">
			...
			>
				<span class="sr-only">poprzednia {{ title }}</span>
			...
			<AdminVButton
				...
				@click="changeOffset(1)"
			>
				<span class="sr-only">następna {{ title }}</span>
				...
				>
					...
					<td role="cell">
						<div class="relative h-full flex items-center gap-2 md:w-full md:justify-around md:gap-0">
							<AdminVLoading v-show="loadingItemId === item.id" class="absolute left-1/2 top-1/2 translate-center" />
							<AdminVButton
		...
		<AdminVLoading v-show="isLoading" class="absolute inset-0 bg-black/20 dark:bg-black/30" size="40" />
	</div>
</template>
...
@media (max-width: 767px) {
	...
	.vTable tr {
		display: block;
		padding-block: clamp(0.5rem, 0.23rem + 1.34vw, 0.88rem);
	}
