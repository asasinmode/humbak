...
async function loadSlides(language: string) {
	...
	try {
		slides.value = await useApi('/api/admin/slides/public', { query: { language } });
...
<template>
	<article
		...
		tabindex="-1"
	>
		<AdminVLoading v-if="isLoading" class="absolute inset-0" />
		<p v-else-if="!slides.length" class="absolute inset-0 grid place-items-center text-lg">
			brak slideów
		</p>
