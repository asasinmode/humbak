...
test('file deleting', async (t) => {
	await t.test('deletes files', async (t) => {
		assert.equal(1, 1);
	});
	...
	await t.test('return associated pages\' ids', async (t) => {
		assert.equal(1, 1);
	});
});

// todo 338, 219 attach original to new
// maybe attach target too
test('file editing', async (t) => {
	await t.test('skips target dir not found in all dirs', async (t) => {
		assert.equal(1, 1);
	});

	// if target attached this isnt needed
...
test('dir editing', async (t) => {
	await t.test('skips self not found in all dirs', async (t) => {
		assert.equal(1, 1);
	});
